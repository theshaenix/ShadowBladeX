...
static bool should_skip_mm(struct mm_struct *mm, struct mm_walk_args *args)
{
	...
	if (size < max((unsigned long)SWAP_CLUSTER_MAX,
		       (unsigned long)atomic_long_read(&mm->nr_ptes)))
		return true;
...
/*
 * 4.14 lacks pmd_leaf/pud_leaf/p4d_leaf. They are used here only in debug
 * assertions and a 1GB-huge-pud skip that does not occur on this device; the
 * actual THP handling below keys off pmd_trans_huge(). So 0 fallbacks are safe.
 */
#ifndef pmd_leaf
#define pmd_leaf(x)	0
#endif
...
/*
 * 4.14 has no p4d_entry in struct mm_walk and its pagewalk per-VMA model
 * conflicts with get_next_vma(). So we drive walk_pud_range() ourselves, one
 * PGD-bounded range at a time, mirroring what the 5.x walk_page_range() does
 * when only .p4d_entry/.test_walk are set.
 */
static void walk_mm(struct mm_walk_args *args, struct mm_struct *mm)
{
	...
	do {
		...
		/* find the first walkable VMA at or after start */
		walk.vma = find_vma(mm, start);
		while (walk.vma &&
		       should_skip_vma(walk.vma->vm_start, walk.vma->vm_end, &walk))
			walk.vma = walk.vma->vm_next;

		if (walk.vma) {
			start = max(start, walk.vma->vm_start);
			end = pgd_addr_end(start, mm->highest_vm_end);
			pgd = pgd_offset(mm, start);
			p4d = p4d_offset(pgd, start);
			err = walk_pud_range(p4d, start, end, &walk);
		} else {
			args->next_addr = 0;
			err = 0;
		}
