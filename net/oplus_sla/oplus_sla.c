...
static void init_oplus_sla_weight(struct timeval *ptv, int work_mode)
{
	/*last_weight_tv = tv;*/
	if (ptv) {
		memcpy(&last_weight_tv, ptv, sizeof(struct timeval));
	}
...
static void reset_oplus_sla_calc_speed(struct timeval *ptv)
{
	int time_interval = 0;
	/*time_interval = tv.tv_sec - last_calc_small_speed_tv.tv_sec;*/
	if (ptv) {
		time_interval = ptv->tv_sec - last_calc_small_speed_tv.tv_sec;
	}
...
//add for rate limit function to statistics front uid rtt
static void oplus_rate_limit_rtt_calc(struct timeval *ptv)
{
	int num = 0;
	int sum = 0;
	int time_interval = 0;
	...
	if (ptv) {
		time_interval = ptv->tv_sec - rate_limit_info.last_tv.tv_sec;
	}
	if(time_interval >= 10){
		sla_rtt_write_lock();
		...
		if (ptv) {
			memcpy(&rate_limit_info.last_tv, ptv, sizeof(struct timeval));
		}
