...
int elevator_init(struct request_queue *q, char *name)
{
	...
	if (!e) {
		...
		/*
		 * Keep the historical blk-mq fallback: single-queue mq devices
		 * use mq-deadline if the configured default is unavailable,
		 * while multi-queue devices fall back to "none".
		 */
		if (!e && q->mq_ops) {
			if (q->nr_hw_queues == 1)
