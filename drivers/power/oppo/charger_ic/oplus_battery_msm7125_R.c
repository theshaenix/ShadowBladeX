...
#define USB_TEMP_DEFAULT_VOLT_VALUE_MV 1285
...
static const struct apsd_result *smblib_get_apsd_result(struct smb_charger *chg)
{
	...
	if (apsd_stat & QC_CHARGER_BIT) {
		...
		if (result != &smblib_apsd_results[HVDCP3]
			&& result->bit == (DCP_CHARGER_BIT | QC_2P0_BIT)) {
			result = &smblib_apsd_results[HVDCP2];
		}
	...
	u8 allowed_voltage)
{
	...
	if (rc < 0)
		smblib_err(chg,
			"Couldn't write 0x%02x to USBIN_ADAPTER_ALLOW_OVERRIDE_REG rc=%d\n",
			allowed_voltage, rc);
	...
	bool enable)
{
	...
	if (enable) {
		...
		if (rc < 0) {
			smblib_err(chg,
				"Couldn't enable moisture detection and uUSB state change interrupt rc=%d\n",
				rc);
			return rc;
		}
	...
	union power_supply_propval *val)
{
	...
	stat = stat & BATTERY_CHARGER_STATUS_MASK;
	val->intval = (stat == TERMINATE_CHARGE) | (g_oplus_chip->batt_full == true);
	return 0;
}
	...
	const union power_supply_propval *val)
{
	...
	smblib_dbg(chg, PR_MISC,
		"snk_attached = %d, src_attached = %d, is_pr_lock = %d\n",
	...
	if (!chg->pr_lock_in_progress && is_pr_lock) {
		...
		smblib_dbg(chg, PR_MISC,
			"restore type-c interrupts after exit power role locking\n");
...
void smblib_usb_plugin_hard_reset_locked(struct smb_charger *chg)
{
	...
	if (vbus_rising) {
		...
		printk(KERN_ERR "!!! %s: the hvdcp_detach_time:%lu\n", __func__,
			chg->hvdcp_detach_time);

		if (!(chg->hvdcp_detect_ok
				&& (chg->hvdcp_detach_time - chg->hvdcp_detect_time <=
...
#else
		if (chg->use_extcon
			|| (smblib_get_prop_ufp_mode(chg) == POWER_SUPPLY_TYPEC_NONE))
#endif
...
irqreturn_t dc_plugin_irq_handler(int irq, void *data)
{
	...
	smblib_dbg(chg, (PR_WLS | PR_INTERRUPT),
		"dcin_present= %d, usbin_present= %d, cp_reason = %d\n",
...
#ifdef OPLUS_FEATURE_CHG_BASIC
/* zhouhaikang@BSP.CHG.Basic,2020/10/20,add for usbtemp recover */
...
int smblib_init(struct smb_charger *chg)
{
	...
	INIT_DELAYED_WORK(&chg->set_chargerid_to_mcu_work,
		oplus_chg_set_chargerid_to_mcu_work);
...
static void typec_disable_cmd_work(struct work_struct *work)
{
	...
	rc = smblib_masked_write(chg, TYPE_C_MODE_CFG_REG, TYPEC_DISABLE_CMD_BIT,
...
static void oplus_chg_monitor_work(struct work_struct *work)
{
	...
	if (chip->icharging >= 0) {
		...
		&& (chip->icharging * -1) <= chip->limits.iterm_ma / 2) {
		counts++;
	...
	if (counts >= (chip->batt_full ? 8 : 3)) {//because rechg counts=6
		...
		if (stat & PRE_TERM_BIT) {
			...
			if (rc < 0) {
				printk(KERN_ERR
					"oplus_chg_monitor_work: Couldn't clear USBIN_SUSPEND_BIT rc=%d\n", rc);
				goto rerun_work;
			}
...
int oplus_charger_int_gpio_init(struct oplus_chg_chip *chip)
{
	...
	charger_int_default = pinctrl_lookup_state(charger_int_pinctrl,
			"charger_int_default");
...
int oplus_charger_error_gpio_init(struct oplus_chg_chip *chip)
{
	...
	charger_error_default = pinctrl_lookup_state(charger_error_pinctrl,
...
int oplus_turn_usbtemp_to_volt(int temp, const struct vadc_map_temp *table,
	int length)
{
	int volt = -1;
	int i = 0;
	...
	volt = (temp - table[i - 1].temp) * (table[i].volt - table[i - 1].volt) /
		(table[i].temp - table[i - 1].temp) + table[i - 1].volt;
	return volt;
}
...
/*chenhongbin@BSP.CHG.usbtemp. 2020/11/14 add for project 19721 usbtemp read */
		for (i = 0; i < 3; i++) {
			...
			if (rc < 0) {
				chg_err("[OPLUS_CHG][%s]: iio_read_channel_processed  get error\n", __func__);
				chip->usbtemp_volt_l = USB_TEMP_DEFAULT_VOLT_VALUE_MV;
				goto usbtemp_next;
			}


			usbtemp = usbtemp / 1000;
			usbtemp_volt += usbtemp;
			if(usbtemp_debug & OPEN_LOG_BIT)
				chg_err("temp_volt_l:%d, usbtemp_volt:%d\n",usbtemp, usbtemp_volt);
				msleep(5);
		}
		chip->usbtemp_volt_l = usbtemp_volt / 3;
		if (chip->usbtemp_volt_l > USB_TEMP_DEFAULT_VOLT_VALUE_MV) {
			chip->usbtemp_volt_l = USB_TEMP_DEFAULT_VOLT_VALUE_MV;
		}
	}
usbtemp_next:
	...
	if (project != 19721) {
		...
		if (rc < 0) {
			...
			chip->usbtemp_volt_r = USB_TEMP_DEFAULT_VOLT_VALUE_MV;
			return;
		}
		...
		if (chip->usbtemp_volt_r > USB_TEMP_DEFAULT_VOLT_VALUE_MV) {
			chip->usbtemp_volt_r = USB_TEMP_DEFAULT_VOLT_VALUE_MV;
		}
	}
...
/*chenhongbin@BSP.CHG.usbtemp. 2020/11/14 add for project 19721 usbtemp read */
	if (project == 19721) {
		...
		resistance_convert_temperature(resistance1, usbtemp_l, index, ntc_table_100K_19721);
		resistance_convert_temperature(resistance2, usbtemp_r, index, ntc_table_100K_19721);

		chip->usb_temp_l = usbtemp_l/10;
		chip->usb_temp_r = usbtemp_r/10;
...
static int oplus_dischg_gpio_init(struct oplus_chg_chip *chip)
{
	...
	chip->normalchg_gpio.dischg_enable = pinctrl_lookup_state(
			chip->normalchg_gpio.pinctrl, "dischg_enable");
	...
	chip->normalchg_gpio.dischg_disable = pinctrl_lookup_state(
			chip->normalchg_gpio.pinctrl, "dischg_disable");
...
static int oplus_chg_parse_custom_dt(struct oplus_chg_chip *chip)
{
	...
	if (g_oplus_chip) {
		g_oplus_chip->normalchg_gpio.dischg_gpio = of_get_named_gpio(node,
				"qcom,dischg-gpio", 0);
...
static int oplus_get_otg_online_status(void)
{
	...
	rc = power_supply_get_property(chg->usb_psy, POWER_SUPPLY_PROP_TYPEC_MODE,
			&val);
...
static void oplus_set_otg_switch_status(bool value)
{
	...
	if (vbus_rising == true) {
		chg_debug("vbus_rising, stat 0x1544 = 0x%x, otg_switch = %d, otg_online = %d, ui_otg_switch = %d\n",
			stat, g_oplus_chip->otg_switch, g_oplus_chip->otg_online,
			g_oplus_chip->ui_otg_switch);
		return;
	}
	...
	union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_ONLINE:
		...
		if (g_oplus_chip && (g_oplus_chip->vbatt_num == 2 || is_vooc_support_single_batt_svooc() == true)) {
	...
	case POWER_SUPPLY_PROP_FAST_CHG_TYPE:
		...
		if (val->intval == 0 && g_oplus_chip
			&& g_oplus_chip->chg_ops->get_charger_subtype) {
...
#ifdef OPLUS_FEATURE_CHG_BASIC
//zhouhaikang@BSP.CHG.Basic,2020/11/2,charge_done to keep msoc FULL_SOC(100%) while battery FULL&IBAT=0. */
	...
	enum power_supply_property psp)
{
	...
	case POWER_SUPPLY_PROP_STEP_CHARGING_ENABLED:
	case POWER_SUPPLY_PROP_DIE_HEALTH:
#ifdef VENDOR_EDIT//tongfeng.huang@PSW.BSP.CHG, 2019/09/24, Add for soc save
	case POWER_SUPPLY_PROP_SOC_NOTIFY_READY:
	case POWER_SUPPLY_PROP_RESTORE_SOC:
	case POWER_SUPPLY_PROP_BATTERY_INFO:
	case POWER_SUPPLY_PROP_BATTERY_INFO_ID:
...
static ssize_t dump_registers_mask_write(struct file *file,
...
{
	if (!proc_create("d_reg_mask", S_IWUSR | S_IWGRP | S_IWOTH, NULL,
			&dump_registers_mask_fops)) {
...
static void smbchg_aicl_enable(bool enable)
{
	...
	rc = smblib_masked_write(&chip->pmic_spmi.smb5_chip->chg,
...
static bool oplus_chg_is_suspend_status(void)
{
	...
	if (rc < 0) {
		printk(KERN_ERR
			"oplus_chg_is_suspend_status: Couldn't read POWER_PATH_STATUS rc=%d\n", rc);
		return false;
	}
...
static int oplus_chg_set_input_current(int current_ma)
{
	...
	if (get_client_vote(chip->pmic_spmi.smb5_chip->chg.usb_icl_votable,
			BOOST_BACK_VOTER) == 0
		&& get_effective_result(chip->pmic_spmi.smb5_chip->chg.usb_icl_votable) <=
		USBIN_25MA) {
		vote(chip->pmic_spmi.smb5_chip->chg.usb_icl_votable, BOOST_BACK_VOTER, false,
			0);
	}
	...
	if (oplus_chg_is_suspend_status() && oplus_chg_is_usb_present()
		&& oplus_chg_is_normal_mode()) {
		i = i - 1;
		goto aicl_suspend;
	}
...
aicl_return:
	...
	if (chip->pmic_spmi.smb5_chip->chg.typec_mode ==
		POWER_SUPPLY_TYPEC_SINK_AUDIO_ADAPTER) {
		...
		if ((bool)(stat & ICL_OVERRIDE_AFTER_APSD_BIT)) {
			rc = smblib_write(&chip->pmic_spmi.smb5_chip->chg, USBIN_CURRENT_LIMIT_CFG_REG,
					0x14);
...
static int smbchg_float_voltage_set(int vfloat_mv)
{
	...
	rc = vote(chip->pmic_spmi.smb5_chip->chg.fv_votable,
			BATT_PROFILE_VOTER/*DEFAULT_VOTER*/,
			true, vfloat_mv * 1000);
...
static int smbchg_get_charge_enable(void)
{
	...
	rc = smblib_read(&chip->pmic_spmi.smb5_chip->chg, CHARGING_ENABLE_CMD_REG,
			&temp);
...
static int smbchg_read_full(void)
{
	...
	rc = smblib_read(&chip->pmic_spmi.smb5_chip->chg, BATTERY_CHARGER_STATUS_1_REG,
			&stat);
...
bool oplus_usb_or_otg_is_present(void)
{
	...
	rc = smblib_read(&chip->pmic_spmi.smb5_chip->chg,
...
bool oplus_chg_is_usb_present(void)
{
	...
	if ((chip->pmic_spmi.smb5_chip->chg.typec_mode == POWER_SUPPLY_TYPEC_SINK
			|| chip->pmic_spmi.smb5_chip->chg.typec_mode ==
...
int oplus_chg_set_pd_config(void)
{
	...
	if (chip->vbatt_num == 2 || is_vooc_support_single_batt_svooc() == true) {
...
void oplus_chg_set_chargerid_to_mcu_work(struct work_struct *work)
{
	...
	if ((oplus_chg_get_charger_subtype() !=  CHARGER_SUBTYPE_QC)
		&& (smbchg_get_chargerid_switch_val() == 0)) {
...
void oplus_chg_reset_mcu_work(struct work_struct *work)
{
	...
	if (!is_fastchg && oplus_chg_get_charger_subtype() != CHARGER_SUBTYPE_PD
		&& oplus_chg_get_charger_subtype() != CHARGER_SUBTYPE_QC) {
...
int oplus_chg_enable_qc_detect(void)
{
	...
	schedule_delayed_work(&chg->set_chargerid_to_mcu_work,
		msecs_to_jiffies(JUDGE_SUBTYPE_QC));
...
int oplus_chg_set_qc_config(void)
{
	...
	if (chip->limits.vbatt_pdqc_to_5v_thr > 0 && chip->charger_volt > 7500
		...
		vooc_enable_cp_for_pdqc();
		msleep(300);
		ret = smblib_masked_write(chg, CMD_HVDCP_2_REG, FORCE_5V_BIT,
...
static int smb5_probe(struct platform_device *pdev)
{
	...
	rc = of_property_match_string(chg->dev->of_node, "io-channel-names",
...
static void smb5_shutdown(struct platform_device *pdev)
{
	...
	/* chenhongbin@BSP.CHG.Basic, 2021/01/15, 20711 need open rt5125 and qcom ship mode */
		if(g_oplus_chip->ship_mode_config == SHIP_MODE_PLATFORM) {
